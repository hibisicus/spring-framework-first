...
//		parentName 可能为空
		...
		if (className != null) {
			if (classLoader != null) {
//				如果classLoader 不为空,则可以使用传入的classLoader同一虚拟机加载类对象,否则只是记录className
	 ...
	 *
	 * 解析的beanName都会被注册到BeanDefinitionRegistry类型的实例registry中,
	 */
...
//		使用beanName作为唯一标识
		...
		// Register aliases for bean name, if any.;如果存在，注册所有别名

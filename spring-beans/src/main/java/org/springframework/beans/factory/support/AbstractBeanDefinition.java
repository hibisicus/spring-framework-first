...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 *默认作用域名称的常量：{@code ""}，相当于单例状态，除非被父 bean 定义覆盖（如果适用）。
	...
	* */
	...
	/*
	* autowireCandidate属性设置为false,这样容器在查找自动装配对象时，将不考虑该Bean,即他
	* 不会被考虑作为其他Bean自动装配的候选者,但是该Bean本身还是可以使用自动装配来注入其他Bean的
	* 对应Bean属性为autowire-Candidate
	* */
	...
	* */
	...
	/*
	* 是否以一种宽松的模式 解析构造函数,默认是true
	* 如果为false,则在以下情况
	* interface ITest{};
	* public ITestImpl implements ITest{};
	* class Main(){
	* 	Main(ITest i);
	* 	Main(ITestImpl i);
	* }
	* 程序抛出异常,因为Spring无法确定哪个构造函数
	* 程序设置
	* */
	private boolean lenientConstructorResolution = true;

	/*
	* 对应 bean属性 factory-bean, 用法 ．
	...
	* */
	...
	/*
	* 方法重写持有者,记录lookup-method,replaced-method等元素
	* */
	private MethodOverrides methodOverrides = new MethodOverrides();

	/*
	* 初始化方法,对应bean属性init-mehtod
	* */
	@Nullable
	private String initMethodName;

	/*
	* 销毁方法,对应bean属性destroy-method方法
	* */
	@Nullable
	...
	* */
	...
	/*
	* 是否使用户定义的，而不是盈盈程序本身定义的，创建AOP的时候为true,程序设置
	* */
	private boolean synthetic = false;

	/*
	* 应以这个Bean的应用，Application:用户,infrastructure:内部完全调用，与用户无关，
